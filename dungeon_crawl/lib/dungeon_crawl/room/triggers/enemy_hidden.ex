defmodule DungeonCrawl.Room.Triggers.EnemyHidden do
  ...
  def run(character, %Action{id: :rest}) do
      ...
      Shell.info("Você encontrou uma sala perfeita para descançar.")
      Shell.info("Mas de repente...")
      Shell.info(enemy.description)
      Shell.info("O inimigo #{enemy.name} surpreendeu você e atacou primeiro.")

      { _, updated_char } = DungeonCrawl.Battle.fight(enemy, character)
      {
        updated_char,
        :forward
      }
